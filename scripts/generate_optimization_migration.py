...
Creates indexes, constraints, and performance optimizations
"""
import os
import sys
import datetime
...
def generate_optimization_migration() -> str:
    """Generate the optimization migration file content"""
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    revision_id = f"{timestamp}_add_performance_indexes"
    prev_revision = f"{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}_initial_schema"
    
    template = f'''"""Add performance indexes and optimizations
...
def upgrade() -> None:
    ...
    op.create_index('idx_discovery_sessions_created', 'discovery_sessions', [sa.text('created_at DESC')])
    ...
    op.execute("""
        ...
        ON track_sources (track_id, source_type, external_id)
    """)
    
    op.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS idx_artist_sources_unique 
        ON artist_sources (artist_id, source_type, external_id)
    """)
    
    op.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS idx_album_sources_unique 
        ON album_sources (album_id, source_type, external_id)
    """)
    ...
    op.execute("ALTER TABLE albums ADD CONSTRAINT check_track_count CHECK (track_count >= 0)")
...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE albums DROP CONSTRAINT IF EXISTS check_track_count")
    ...
    indexes_to_drop = [
        ...
        # Time-based indexes
        'idx_user_interactions_created', 'idx_discovery_sessions_created',
        ...
        # Partial indexes
        'idx_tracks_high_energy', 'idx_tracks_danceable', 'idx_playlists_public_collab',
...
def main():
    """Main function"""
    print("Generating optimization migration...")
    
    migration_content = generate_optimization_migration()
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{timestamp}_add_performance_indexes.py"
    filepath = os.path.join(migration_dir, filename)
    
    # Update the down_revision to match the actual initial migration
    import glob
    initial_migrations = glob.glob(os.path.join(migration_dir, "*_initial_schema.py"))
    if initial_migrations:
        with open(initial_migrations[0], 'r') as f:
            for line in f:
                if line.startswith("revision = "):
                    initial_revision = line.split("'")[1]
                    migration_content = migration_content.replace(
                        "down_revision = '{}_initial_schema'".format(datetime.datetime.now().strftime('%Y%m%d_%H%M%S')),
    ...
    print(f"Optimization migration created: {filepath}")
    print("\nThis migration adds:")
    print("- 35+ performance indexes")
    print("- JSONB GIN indexes for flexible queries")
    print("- Full-text search indexes")
    print("- Materialized view for track popularity")
